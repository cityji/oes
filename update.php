...
if (isset($_SESSION['key'])) {
    if (@$_GET['fdid'] && $_SESSION['key'] == 'QuizLetADMIN') {
        $id = @$_GET['fdid'];
...
//delete user
if (isset($_SESSION['key'])) {
    if (@$_GET['demail'] && $_SESSION['key'] == 'QuizLetADMIN') {
        $demail = @$_GET['demail'];
...
//remove quiz
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'rmquiz' && $_SESSION['key'] == 'QuizLetADMIN') {
        $eid = @$_GET['eid'];
...
//add quiz
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'QuizLetADMIN') {
        ...
        $id = uniqid();
        // var_dump($json);
        $q3 = mysqli_query($con, "INSERT INTO `quiz`(`eid`, `title`, `sahi`, `wrong`, `total`, `time`, `intro`, `tag`, `date`) VALUES  ('$id','$name' , '$sahi' , '$wrong','$total','$time' ,'$desc','$tag', NOW())");
        // echo ("location:dash.php?q=4&step=2&eid=$id&n=$total&json=" . json_encode($json));
        // stop execution
        header("location:dash.php?q=4&step=2&eid=$id&n=$total&json=" . json_encode($json));
    }
}

//add question
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addqns' && $_SESSION['key'] == 'QuizLetADMIN') {
        $n = @$_GET['n'];
        $eid = @$_GET['eid'];
        $ch = @$_GET['ch'];
        for ($i = 1; $i <= $n; $i++) {
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2) {
    ...
    } else if ($_SESSION['key'] != 'QuizLetADMIN') {
